...
<head>
  ...
  <title>HR Application System - Utilities(Loan Section)</title>
      ...
      <nav>
        <div class="d-flex flex-column flex-shrink-0 p-3 bg-light">
          <ul class="nav nav-pills flex-column mb-auto">
            ...
            <li>
              <a href="${pageContext.request.contextPath}/manager/loansection" class="nav-link active" aria-current="page">
        ...
        <table id="table" class="table table-responsive table-borderless" 
          ...
          <thead>
            <tr class="border-bottom">
              ...
              <th data-field="applying-date">Applying Date</th>
          ...
          <tbody>
            <c:forEach var="loan" items="${loanList}">
              ...
              <img src="${pageContext.request.contextPath}/img/approved.png" alt="approved" class="status-btn"/>
              </c:when>
              <c:when test="${loan.status == 'REJECTED'}">
              <img src="${pageContext.request.contextPath}/img/rejected.png" alt="rejected" class="status-btn"/>
              </c:when>
              <c:otherwise>
              <p>${loan.status}</p>
              </c:otherwise>
                ...
                <td>${loan.approver}</td>
                <td>
                  <button type="button" class="btn btn-outline-info btn-sm" onclick="location.href='${pageContext.request.contextPath}/manager/updateEmployeeLoan?id=${loan.id}&status=APPROVED&by=${user.fullname}'">Approved</button>
                  <button type="button" class="btn btn-outline-danger btn-sm" onclick="location.href='${pageContext.request.contextPath}/manager/updateEmployeeLoan?id=${loan.id}&status=REJECTED&by=${user.fullname}'">Rejected</button>
                </td>
              </tr>
            </c:forEach>
          </tbody>
