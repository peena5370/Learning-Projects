...
<head>
  ...
  <title>HR Application System - Utilities(Training Section)</title>
...
<body>
  ...
  <div class="bootstrap-table bootstrap5 container-bg" style="margin: 0 5px 0 5px;">
    <div class="row mb-5 mt-3">
      <h3 class="text-center">Employee Training List</h3>
      <hr>
    </div>
    <div class="float-left" style="margin-left: 10px;">
      <button class="btn btn-primary" style="border-radius: 20px;"onclick="openWindow('${pageContext.request.contextPath}/v2/manager/form/issueTraining')">
      <i class="fa fa-plus"></i>&nbsp;&nbsp;Issue Training
      </button>
    </div>
    ...
    data-height="620">
      <thead class="sky-blue">
        <tr>
          <th data-field="id">#</th>
          <th data-field="reasons">Training Title</th>
          ...
          <th data-field="issuer">Issued By</th>
        </tr>
      </thead>
      <tbody>
        <c:forEach var="training" items="${trainingList}">
          <tr class="fw-light">
            <th scope="row" class="fw-normal">${training.id}</th>
            <td>${training.info}</td>
            <td>${training.startDate}</td>
            <td>${training.endDate}</td>
            <td>
            	<c:choose>
					<c:when test="${training.status == 'COMPLETED'}">
					<img src="${pageContext.request.contextPath}/resources/images/completed.png" alt="completed" class="status-btn-2"/>
					</c:when>
					<c:when test="${training.status == 'PENDING'}">
					<img src="${pageContext.request.contextPath}/resources/images/pending.png" alt="pending" class="status-btn-2"/>
					</c:when>
					<c:otherwise>
					<p>${training.status}</p>
					</c:otherwise>
