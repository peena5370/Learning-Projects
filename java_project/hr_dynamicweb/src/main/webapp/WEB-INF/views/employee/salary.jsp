...
<meta name="viewport" content="width=device-width, initial-scale=1">
  <title>HR Application System - Monthly Salary Information</title>
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav me-auto mb-2 my-lg-0">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="employee?salary">Salary</a>
        </li>
        <li class="nav-item">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        <c:forEach var="salary" items="${salary_list}">
          <tr class="fw-light">
            <th scope="row" class="fw-normal">${salary.monthlysalary_id}</th>
            <td>${salary.current_salary}</td>
            <td>${salary.overtime_pay}</td>
            <td>${salary.deduction}</td>
            <td>${salary.total_salary}</td>
            <td>${salary.payment_date}</td>
            <td>${salary.approvedByManager}</td>
          </tr>
        </c:forEach>
      </tbody>
    </table>
