...
<meta name="viewport" content="width=device-width, initial-scale=1">
  <title>HR Application System - Employees Monthly Salary List</title>
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  ...
  <div class="bootstrap-table bootstrap5"  style="margin-left: 5px; margin-right: 5px;">
    <div class="row mb-5 mt-2">
  	  <h3 class="text-center">Employees Monthly Salary List</h3>
    </div>
    <table id="table" data-toggle="table" data-show-columns="true" data-show-columns-toggle-all="true" data-pagination="true" data-search="true" data-search-highlight="true" data-height="620" class="table table-bordered table-hover">
      <thead class="water">
        <tr>
          ...
          <th data-field="current_salary">Current Base Salary</th>
          <th data-field="ot_pay">OT Pay</th>
          <th data-field="deduction">Deduction</th>
          <th data-field="total_salary">Total Salary</th>
          <th data-field="employee_name">Employee Name</th>
          <th data-field="approved_by">Approved By</th>
          <th data-field="issued_date">Issue Date</th>
        </tr>
      </thead>
      <tbody>
        <c:forEach var="payment" items="${viewPayment}">
          <tr class="fw-light">
            <th scope="row" class="fw-normal">${payment.monthlysalary_id}</th>
            <td>${payment.current_salary}</td>
            <td>${payment.overtime_pay}</td>
            <td>${payment.deduction}</td>
            <td>${payment.total_salary}</td>
            <td>${payment.employee_name}</td>
            <td>${payment.approvedByManager}</td>
