...
<meta name="viewport" content="width=device-width, initial-scale=1">
  <title>HR Application System - Employee Salary List</title>
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <table id="table" data-toggle="table" data-show-columns="true" data-show-columns-toggle-all="true" data-pagination="true" data-search="true" data-search-highlight="true" data-height="620" class="table table-bordered table-hover">
      <thead class="water">
        <tr>
          ...
          <th data-field="base_salary">Base Salary</th>
          <th data-field="incremented_salary">Incremented Salary</th>
          <th data-field="increment_date">Increment Date</th>
          <th data-field="employee_name">Employee Name</th>
          <th data-field="approved_by">Increment Approved By</th>
          <th data-field="action">Action</th>
        </tr>
      </thead>
      <tbody>
        <c:forEach var="salary" items="${viewSalary}">
          <tr class="fw-light">
            <th scope="row" class="fw-normal">${salary.s_id}</th>
            <td>${salary.base_salary}</td>
            <td>${salary.incremented_salary}</td>
            <td>${salary.increment_date}</td>
            <td>${salary.employee_name}</td>
            <td>${salary.issuedByManager}</td>
            <td>
              <button type="button" class="btn btn-outline-primary btn-sm" onclick="openWindow('issueIncrement?name=${salary.employee_name}&user=${sessionScope.username}'); return false;">Issue Increment</button>
            </td>
          </tr>
        </c:forEach>
      </tbody>
  ...
  <script>
    function openWindow(url) {
      openWindow = window.open(url,'_blank','height=400,width=600,left=250,top=250,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes');
	  }
  </script>
</body>
</html>