...
async def main():
    ...
    with next(get_db()) as db:
        ...
        if(create_role):
            print(f"created role_id={create_role.role_id}, role_name={create_role.role_name}")
            get_role = sync_service.get_role_by_id(db, create_role.role_id) # type: ignore
            if(get_role):
                print(f"get created role by id={create_role.role_id}, get_role_id={get_role.role_id}, role_name={get_role.role_name}")

            update_role = sync_service.update_role(db, create_role.role_id, "new update name")  # type: ignore
            if(update_role):
                print(f"updated role: role_id={update_role.role_id}, role_name={update_role.role_name}")
    ...
    async with get_db_session() as db:
        ...
        if (create_role):
            print(f"created async role_id={create_role.role_id}, role_name={create_role.role_name}")

            get_role = await async_service.get_role_by_id(db, create_role.role_id)  # type: ignore
            if (get_role):
                print(f"get created async role by id={create_role.role_id}, get_role_id={get_role.role_id}, role_name={get_role.role_name}")
            
            update_role = await async_service.update_role(db, create_role.role_id, "new async update name")  # type: ignore
            if (update_role):
                print(f"updated async role: role_id={update_role.role_id}, role_name={update_role.role_name}")
