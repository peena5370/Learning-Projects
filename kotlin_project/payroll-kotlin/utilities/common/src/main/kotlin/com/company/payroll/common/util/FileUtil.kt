 ...
 */
@Component
class FileUtil(@Value("\${file.upload.directory}") directory: String)
{
...
//  }
    ...
    fun imageUpload(file: MultipartFile, imgPath: String): String
    {
        ...
        val regex: Matcher =
            Pattern.compile("(\\w*)(\\.)(jpg|jpeg|png|gif)", Pattern.CASE_INSENSITIVE).matcher(filename)
    ...
    fun fileUpload(file: MultipartFile, path: String): String
    {
        ...
        val filename: String = StringUtils.cleanPath(file.originalFilename.toString())
        val regex: Matcher =
            Pattern.compile("(\\w*\\p{P}.*)|([0-9])|(\\w*)(\\.)(doc|docx|pdf)", Pattern.CASE_INSENSITIVE)
                .matcher(filename)
